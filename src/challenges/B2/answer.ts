...
export default function ({
  ...
  const test = events.reduce((acc: string[], category: EventWithCategory) => {
    return acc.concat(category.categories);
  }, []);
  const uniqueCategories = Array.from(new Set(test)).sort();
